...
DEFAULT_OUT = str(TABLES_DIR)
DEFAULT_FIGURES = str(FIGURES_DIR)
DEFAULT_MODEL_DIR = str(MODELS_DIR)
os.makedirs(DEFAULT_OUT, exist_ok=True)
os.makedirs(DEFAULT_FIGURES, exist_ok=True)
...
def main():
    ...
    if valid_mask_pred.sum() == 0:
        ...
        plt.tight_layout()
        cm_path = os.path.join(DEFAULT_FIGURES, "confusion_matrix.png")
    ...
    if valid_mask_pred.sum() > 0:
        ...
        with open(os.path.join(DEFAULT_FIGURES, "classification_report.txt"), "w") as fh:
            fh.write(report)
