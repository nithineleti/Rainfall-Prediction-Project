...
Behavior:
 - Tries to detect the feature column names used during training by reading:
     1) data/tables/train_samples_clean_nogrp.csv (preferred)
     2) features_bands.csv (fallback)
...
Usage:
    python src/predict_map.py --stack data/rasters/features_stack.tif \
        --model models/rf_baseline.pkl --out_dir outputs/predictions
"""
...
import argparse
import pandas as pd
from path_config import TABLES_DIR, FEATURES_STACK, PREDICTIONS_DIR

DEFAULT_TRAIN_CSV = os.path.join(str(TABLES_DIR), "train_samples_clean_nogrp.csv")
DEFAULT_BANDCSV = os.path.join(str(TABLES_DIR), "features_bands.csv")
DEFAULT_OUT_DIR = str(PREDICTIONS_DIR)
