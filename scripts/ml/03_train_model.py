...
Outputs:
 ...
 - data/figures/classification_report.txt

Usage:
    python src/train_model.py --in data/tables/train_samples.csv --out_dir models --cv_k 5
"""
import os
import sys
import argparse
...
from path_config import TABLES_DIR, FIGURES_DIR, MODELS_DIR

OUTPUT_TABLES = str(TABLES_DIR)
OUTPUT_FIGURES = str(FIGURES_DIR)
...
def main():
    ...
    recdf.to_csv(os.path.join(OUTPUT_TABLES, "cv_results.csv"), index=False)
    print("Saved CV results:", os.path.join(OUTPUT_TABLES, "cv_results.csv"))
    ...
    fi_df.to_csv(os.path.join(OUTPUT_TABLES, "feature_importances.csv"), index=False)
    print("Saved feature importances:", os.path.join(OUTPUT_TABLES, "feature_importances.csv"))
    ...
    if valid_mask_pred.sum() == 0:
        ...
        plt.tight_layout()
        cm_path = os.path.join(OUTPUT_FIGURES, "confusion_matrix.png")
    ...
    if valid_mask_pred.sum() > 0:
        ...
        with open(os.path.join(OUTPUT_FIGURES, "classification_report.txt"), "w") as fh:
            fh.write(report)
        print("Saved classification report:", os.path.join(OUTPUT_FIGURES, "classification_report.txt"))
